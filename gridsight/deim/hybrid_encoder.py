...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if self.fuse_op not in {'sum', 'cat', 'weighted_sum'}:
            raise ValueError(
                f"Unsupported fuse_op={self.fuse_op!r}; expected 'sum', "
                "'cat', or 'weighted_sum'."
            )
        input_dim = hidden_dim if self.fuse_op in {'sum', 'weighted_sum'} else hidden_dim * 2
        ...
        if self.fuse_op == 'weighted_sum':
            self.fpn_fusion_weights = nn.ParameterList([
                nn.Parameter(torch.ones(2, dtype=torch.float32))
                for _ in range(len(in_channels) - 1)
            ])
            self.pan_fusion_weights = nn.ParameterList([
                nn.Parameter(torch.ones(2, dtype=torch.float32))
                for _ in range(len(in_channels) - 1)
            ])
    ...
    @staticmethod
    def _weighted_sum(first, second, weights, eps=1e-4):
        positive_weights = F.relu(weights)
        normalized_weights = positive_weights / (positive_weights.sum() + eps)
        # Multiplication by two preserves the magnitude of the legacy
        # ``first + second`` path when both weights are initialized equally.
        return 2.0 * (
            normalized_weights[0] * first + normalized_weights[1] * second
        )
    ...
    def forward(self, feats):
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            elif self.fuse_op == 'weighted_sum':
                fused_feat = self._weighted_sum(
                    upsample_feat, feat_low, self.fpn_fusion_weights[fusion_idx]
                )
            else:
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            if self.fuse_op == 'sum':
                ...
                fused_feat = self._weighted_sum(
                    downsample_feat, feat_height, self.pan_fusion_weights[idx]
                )
            else:
