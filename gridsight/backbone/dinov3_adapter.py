...
class DINOv3STAs(nn.Module):
    def __init__(
            ...
            if weights_path is not None:
                if not os.path.isfile(weights_path):
                    raise FileNotFoundError(
                        ...
                        'Please provide a Meta-native .pth state_dict.'
                    )
                print(f'Loading ckpt from {weights_path}...')
                state_dict = torch.load(weights_path, map_location='cpu', weights_only=True)
                self.dinov3.load_state_dict(state_dict, strict=True)
            else:
        ...
        if self.semantic_fusion not in {'legacy', 'adaptive'}:
            raise ValueError(
                f"Unsupported semantic_fusion={self.semantic_fusion!r}; "
                "expected 'legacy' or 'adaptive'."
            )
        if self.semantic_fusion == 'adaptive':
            if len(self.interaction_indexes) != 3:
                raise ValueError(
                    "Adaptive semantic fusion requires exactly three "
                    "interaction indexes."
                )
            # Each output scale initially follows the corresponding legacy
            # intermediate layer while retaining a small contribution from
            # the other two layers. The weights are normalized in forward.
            self.layer_fusion_logits = nn.Parameter(torch.eye(3) * 4.0)
    ...
    def forward(self, x):
        ...
        base_sem_feats = []
        for sem_feat in all_layers:
            feat, _ = sem_feat
            sem_feat = feat.transpose(1, 2).view(bs, -1, H_c, W_c).contiguous()  # [B, D, H, W]
            base_sem_feats.append(sem_feat)

        if self.semantic_fusion == 'adaptive':
            fusion_weights = self.layer_fusion_logits.softmax(dim=1)
            stacked_sem_feats = torch.stack(base_sem_feats, dim=1)
            base_sem_feats = [
                torch.sum(
                    stacked_sem_feats * fusion_weights[i].view(1, -1, 1, 1, 1),
                    dim=1,
                )
                for i in range(3)
            ]
        ...
        sem_feats = []
        num_scales = len(base_sem_feats) - 2
        for i, sem_feat in enumerate(base_sem_feats):
